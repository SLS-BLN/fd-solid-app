...
const TodoApp = () => {
  ...
  const [state, setState] = createLocalStore({
    ...
    addTodo = () => {
      if (newEditTitle.value) {
        setState({
          todos: [
            {
              title: newEditTitle.value,
              due: newEditDue.value,
              id: state.counter,
              completed: false,
            },
            ...state.todos,
    ...
    toggleTodo = (id) => {
      const currentTodo = state.todos.find((todo) => {
        return todo.id === id;
      });
    ...
    languageSelected = (event) => {
      ...
      url.search = `?sap-ui-language=${
        event.detail.targetItem.dataset.theme || "en"
      }`;
      document.location.href = url.toString();
    },
    openGitHub = () => {
      window.open(
        "https://github.com/petermuessig/solidjs-todoapp",
        "_blank",
        "noopener"
      );
    };
    ...
    <section class="todo-wrapper">
      <header>
        <ui5-shellbar primary-title={i18nBundle.getText("TITLE")}>
          ...
          <ui5-shellbar-item
            icon="world"
            text={i18nBundle.getText("LANGUAGE")}
            on:click={selectLanguage}
          />
          <ui5-shellbar-item
            icon="palette"
            text={i18nBundle.getText("THEME")}
          ...
          <ui5-shellbar-item
            ...
            on:click={openGitHub}
          />
          <ui5-avatar slot="profile" size="XS" initials="DR" />
        </ui5-shellbar>
        <LanguageSwitch
          ref={languageSwitch}
          headerText={i18nBundle.getText("LANGUAGE")}
          onLanguageSelected={languageSelected}
        />
        <ThemeSwitch
          ref={themeSwitch}
          headerText={i18nBundle.getText("THEME")}
      ...
      <section class="todo">
        <div class="todo__create fd-layout-panel">
          <div class="todo__create-wrapper fd-layout-panel__body">
            <ui5-input
              class="todo__task-name"
              ref={newEditTitle}
              placeholder={i18nBundle.getText("TODO_ENTER")}
            />
            <ui5-date-picker
              class="todo__task-date"
              ref={newEditDue}
            ...
            <ui5-button
              ...
              design="Emphasized"
            >
              {i18nBundle.getText("TODO_ADD")}
            </ui5-button>
          </div>
        </div>

        <ui5-panel
          header-text={i18nBundle.getText("TODO_INCOMPLETE")}
          collapsed
          class="todo__list"
        >
          <TodoList>
            <For each={filterTodos()}>
              {(todo) => (
                <TodoListItem
                  {...todo}
                  onToggle={toggleTodo}
                  editText={i18nBundle.getText("TODO_EDIT")}
                  onEdit={editTodo}
                  deleteText={i18nBundle.getText("TODO_DELETE")}
                  onDelete={deleteTodo}
                ></TodoListItem>
              )}
            </For>
        ...
        <ui5-panel
          header-text={i18nBundle.getText("TODO_COMPLETED")}
          collapsed
          class="todo__list"
        >
          <TodoList>
